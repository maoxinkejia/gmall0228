...
/**
 * 拦截器，用来拦截所有的请求，在到达目标地址之前需要进行身份验证（token验证）
 * 如果验证通过则继续跳转页面，如果验证失败则跳转至登录页面
 */
//拦截器配置类注入时需要被扫描，添加扫描注解
@Component
public class AuthInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 用户登录之后，跳转其他页面，当第二次登录时newToken可能不存在，第一次登录的时候，已经将newToken值放入cookie中
        if (token == null) {
            //根据cookieName 从cookie中取值
